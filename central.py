...
#		lAddList(lSipSport)
		if lSwitch != "None":
			...
			print "Could not locate switch for " + lSip
	else:
...
def audioEnd(lSip, lSport):
	...
	lExist = lCheckList(lSipSport)
	if lExist == True:
		lSwitch, lInt = lSwitchInt(lSip)
                lRemCos(lSwitch, lInt, lSip, lSport)	
		lRemList(lSipSport)
...
lCentralMain("<EVENT><TYPE>audio</TYPE><TRIGGER>Start</TRIGGER><IP1>10.105.5.213</IP1><PORT1>3832</PORT1><IP2>10.105.5.215</IP2><PORT2>5068</PORT2></EVENT>")
